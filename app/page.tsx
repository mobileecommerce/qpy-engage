...
// A conversation list column is narrow; a full "7/27/2026, 9:31:01 AM" wraps onto three lines and
// squeezes the name it sits beside. Today's conversations only need the time, older ones the date.
function conversationTime(when:string|number):string{
...
function WebChatInbox({notify}:{notify:(s:string)=>void}){
  ...
  const loadDocuments=()=>{
    if(!selectedSessionId||!token){setDocuments([]);return}
    fetch(metaApi(`/api/widget/documents?contactKey=${encodeURIComponent(selectedSessionId)}`),{headers:authHeaders(token)})
      .then(r=>r.json())
      .then((data:{documents?:UploadedDocument[]})=>setDocuments(data.documents||[]))
      .catch(()=>{});
  };
  useEffect(()=>loadDocuments(),[selectedSessionId,token]);
  // Downloads need the session token, which a plain <a href> cannot carry — so fetch the file as a
  // blob and hand the browser that instead.
  const downloadDocument=async(doc:UploadedDocument)=>{
    if(!token)return;
    try{
      const response=await fetch(metaApi(`/api/widget/documents/${doc.id}`),{headers:authHeaders(token)});
    ...
    }catch{ notify("Could not download that document"); }
  };
  const loadLead=()=>{
...
function Audiences({notify}:{notify:(s:string)=>void}){
  ...
  return <><PageHeader title="Audiences" description="Manage real contact groups for campaign targeting." action={<button className="primary" onClick={()=>{setShowNewAudience(true)}}>＋ Create audience</button>}/>
    ...
    {loading?<p className="empty-hint">Loading…</p>:!audiences.length&&!showNewAudience?<div className="empty-state"><span>♟</span><h3>No audiences yet</h3><p>Group real contacts together so a campaign can target them.</p><button className="primary" onClick={()=>setShowNewAudience(true)}>Create your first audience</button></div>:!audiences.length?null:
  ...
  return <>
    ...
    <div className="settings-layout"><aside><button className={tab==="flows"?"active":""} onClick={()=>setTab("flows")}>Flows</button><button className={tab==="items"?"active":""} onClick={()=>setTab("items")}>Items catalog</button></aside><section className="subscription-content">
      {loading?<p className="empty-hint">Loading…</p>:tab==="flows"?<>
        {!flows.length?<div className="empty-state"><span>⑃</span><h3>No flows yet</h3><p>Build a step-by-step conversation — dates, choices, an items carousel — that runs the same way every time, with no AI involved.</p><button className="primary" onClick={()=>setEditingFlow({id:"",name:"New flow",triggerText:"",status:"draft",startStepId:"",steps:[]})}>Create your first flow</button></div>:
...
function ChangePassword({notify}:{notify:(s:string)=>void}){
  const token=useAuthToken();
  ...
  // sent anywhere — the server remains the authority.
  const problem = next && next.length<8 ? "New password must be at least 8 characters."
    : next && confirm && next!==confirm ? "The two new passwords do not match."
  ...
  const submit=async()=>{
    ...
    setSaving(true);
    try{
      const response=await fetch(metaApi("/api/auth/password"),{method:"POST",
        headers:{"content-type":"application/json",...authHeaders(token)},
        body:JSON.stringify({currentPassword:current,newPassword:next})});
      const data=await response.json() as {ok?:boolean;error?:string};
      if(!response.ok||!data.ok)throw new Error(data.error||"Could not change the password");
      setCurrent(""); setNext(""); setConfirm(""); setDone(true);
      notify("Password changed — other devices have been signed out");
    }catch(error){ notify(error instanceof Error?error.message:"Could not change the password") }
    finally{ setSaving(false) }
  };

  return <>
    <h2>Password</h2>
    <p>Changing this signs you out everywhere except here.</p>
    <div className="pw-form">
      ...
      <label>Confirm new password
        <input type="password" autoComplete="new-password" value={confirm}
          ...
          onKeyDown={e=>{if(e.key==="Enter"&&ready)submit()}}/></label>
      {problem&&<p className="pw-problem">{problem}</p>}
      {done&&!problem&&<p className="pw-done">Password updated.</p>}
